...
class TestAirSpiderHelpers(unittest.TestCase):

    def test_data_path_should_return_a_string(self):
        data_path = AirSpiderHelpers.dataPath()
        self.assertIsInstance(data_path, str)

    def test_cache_path_should_return_a_string(self):
        cache_path = AirSpiderHelpers.cachePath()
        self.assertIsInstance(cache_path, str)

    def test_log_path_should_return_a_string(self):
        log_path = AirSpiderHelpers.logPath()
        self.assertIsInstance(log_path, str)

    def test_target_type(self):
        target_type = AirSpiderHelpers.targetTypeFromString("0.0.0.0")
        self.assertEqual('IP_ADDRESS', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString("noreply@airspider.net")
        self.assertEqual('EMAILADDR', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString("0.0.0.0/0")
        self.assertEqual('NETBLOCK_OWNER', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString("+1234567890")
        self.assertEqual('PHONE_NUMBER', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString('"Human Name"')
        self.assertEqual('HUMAN_NAME', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString('"abc123"')
        self.assertEqual('USERNAME', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString("1234567890")
        self.assertEqual('BGP_AS_OWNER', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString("::1")
        self.assertEqual('IPV6_ADDRESS', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString("airspider.net")
        self.assertEqual('INTERNET_NAME', target_type)
        target_type = AirSpiderHelpers.targetTypeFromString("1HesYJSP1QqcyPEjnQ9vzBL1wujruNGe7R")
        self.assertEqual('BITCOIN_ADDRESS', target_type)

    def test_target_type_invalid_seed_should_return_none(self):
        target_type = AirSpiderHelpers.targetTypeFromString(None)
        self.assertEqual(None, target_type)

        target_type = AirSpiderHelpers.targetTypeFromString("")
        self.assertEqual(None, target_type)
    ...
    def test_urlRelativeToAbsolute_argument_url_relative_path_should_return_an_absolute_path(self):
        relative_url = AirSpiderHelpers.urlRelativeToAbsolute('/somewhere/else/../../path?param=value#fragment')
    ...
    def test_url_base_dir_should_return_a_string(self):
        base_dir = AirSpiderHelpers.urlBaseDir('http://localhost.local/path?param=value#fragment')
    ...
    def test_url_base_url_should_return_a_string(self):
        base_url = AirSpiderHelpers.urlBaseUrl('http://localhost.local/path?param=value#fragment')
    ...
    def test_dictionaryWordsFromWordlists_should_return_a_set(self):
        words = AirSpiderHelpers.dictionaryWordsFromWordlists()
    ...
    def test_dictionaryWordsFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            AirSpiderHelpers.dictionaryWordsFromWordlists(['does not exist'])

    def test_humanNamesFromWordlists_should_return_a_set(self):
        names = AirSpiderHelpers.humanNamesFromWordlists()
    ...
    def test_humanNamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            AirSpiderHelpers.humanNamesFromWordlists(['does not exist'])

    def test_usernamesFromWordlists_should_return_a_set(self):
        users = AirSpiderHelpers.usernamesFromWordlists()
    ...
    def test_usernamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            AirSpiderHelpers.usernamesFromWordlists(['does not exist'])
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    AirSpiderHelpers.buildGraphData(invalid_type)

    def test_buildGraphData_empty_data_should_raise_ValueError(self):
        with self.assertRaises(ValueError):
            AirSpiderHelpers.buildGraphData([])
    ...
    def test_buildGraphData_should_return_a_set(self):
        graph_data = AirSpiderHelpers.buildGraphData(
            [
    ...
    def test_buildGraphGexf_should_return_bytes(self):
        gexf = AirSpiderHelpers.buildGraphGexf('test root', 'test title', [["test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "ENTITY", "test", "test", "test"]])
        self.assertIsInstance(gexf, bytes)

        self.assertEqual('TBD', 'TBD')

    def test_buildGraphJson_should_return_a_string(self):
        json = AirSpiderHelpers.buildGraphJson('test root', [["test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "ENTITY", "test", "test", "test"]])
    ...
    def test_dataParentChildToTree_invalid_data_type_should_return_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    AirSpiderHelpers.dataParentChildToTree(invalid_type)

    def test_dataParentChildToTree_empty_data_should_return_ValueError(self):
        with self.assertRaises(ValueError):
            AirSpiderHelpers.dataParentChildToTree(dict())

    def test_dataParentChildToTree_should_return_dict(self):
        tree = AirSpiderHelpers.dataParentChildToTree({"test": {"123": "456"}})
        self.assertIsInstance(tree, dict)

    def test_genScanInstanceId_should_return_a_string(self):
        scan_instance_id = AirSpiderHelpers.genScanInstanceId()
    ...
    def test_extractLinksFromHtml_argument_data_not_containing_links_should_return_an_empty_dict(self):
        parse_links = AirSpiderHelpers.extractLinksFromHtml('url', '<html>example html content</html>', 'domains')
    ...
    def test_extractLinksFromHtml_argument_data_containing_malformed_html_with_links_should_return_a_dict_of_urls(self):
        url = 'http://airspider.net/'
        parse_links = AirSpiderHelpers.extractLinksFromHtml(
            url,
            '<!DOCTYPE html><html lang="en-US"><meta charset="UTF-8" /><link rel="pingback" href="http://airspider.net/xmlrpc.php">example html content<unclosed tag><a href="http://airspider.net/path"></a><a href="/relative-path"></a></html>',
            'domains'
        )
        self.assertIsInstance(parse_links, dict)
        ...
        self.assertEqual(
            parse_links,
            {
                'http://airspider.net/xmlrpc.php': {'source': url, 'original': 'http://airspider.net/xmlrpc.php'},
                'http://airspider.net/path': {'source': url, 'original': 'http://airspider.net/path'},
                'http://airspider.net/relative-path': {'source': url, 'original': '/relative-path'},
            }
        )
    ...
    def test_extractLinksFromHtml_invalid_url_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    AirSpiderHelpers.extractLinksFromHtml(invalid_type, 'example html content', 'domains')
    ...
    def test_extractLinksFromHtml_invalid_data_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    AirSpiderHelpers.extractLinksFromHtml("", invalid_type, 'domains')

    def test_extractLinksFromHtml_invalid_domains_should_return_a_dict(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                parse_links = AirSpiderHelpers.extractLinksFromHtml('url', 'example html content', invalid_type)
                self.assertIsInstance(parse_links, dict)
    ...
    def test_extractHashesFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                hashes = AirSpiderHelpers.extractHashesFromText(invalid_type)
    ...
    def test_extractHashesFromText_argument_data_should_return_hahes(self):
        ...
        hashes = AirSpiderHelpers.extractHashesFromText(f"airspider{md5_hash}airspider{sha1_hash}airspider{sha256_hash}airspider{sha512_hash}airspider")
    ...
    def test_valid_email_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_email = AirSpiderHelpers.validEmail(invalid_type)
                self.assertIsInstance(valid_email, bool)
                self.assertFalse(valid_email)

        valid_email = AirSpiderHelpers.validEmail('%@localhost.local')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)
        ...
        valid_email = AirSpiderHelpers.validEmail('root@localhost.local\n.com')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)

        valid_email = AirSpiderHelpers.validEmail('root@localhost')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)
    ...
    def test_validPhoneNumber_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_phone = AirSpiderHelpers.validPhoneNumber(invalid_type)
                self.assertIsInstance(valid_phone, bool)
                self.assertFalse(valid_phone)

        valid_phone = AirSpiderHelpers.validPhoneNumber('+1234567890')
        self.assertIsInstance(valid_phone, bool)
        self.assertFalse(valid_phone)

        valid_phone = AirSpiderHelpers.validPhoneNumber('+12345678901234567890')
        self.assertIsInstance(valid_phone, bool)
        self.assertFalse(valid_phone)

        valid_phone = AirSpiderHelpers.validPhoneNumber('+12345678901')
        self.assertIsInstance(valid_phone, bool)
    ...
    def test_validLEI_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_phone = AirSpiderHelpers.validLEI(invalid_type)
                self.assertIsInstance(valid_phone, bool)
                self.assertFalse(valid_phone)

        valid_lei = AirSpiderHelpers.validLEI('7ZW8QJWVPR4P1J1KQYZZ')
        self.assertIsInstance(valid_lei, bool)
        self.assertFalse(valid_lei)

        valid_lei = AirSpiderHelpers.validLEI('7ZW8QJWVPR4P1J1KQY45')
        self.assertIsInstance(valid_lei, bool)
        self.assertTrue(valid_lei)

    def test_countryCodes_should_return_a_dict(self):
        country_code_dict = AirSpiderHelpers.countryCodes()
    ...
    def test_countryNameFromCountryCode_argument_countryCode_should_return_country_as_a_string(self):
        country_name = AirSpiderHelpers.countryNameFromCountryCode('US')
    ...
    def test_countryNameFromTld_argument_tld_should_return_country_as_a_string(self):
        ...
        for tld in tlds:
            with self.subTest(tld=tld):
                country_name = AirSpiderHelpers.countryNameFromTld(tld)
                self.assertIsInstance(country_name, str)
                self.assertEqual(country_name, "United States")

    def test_extractEmailsFromText_should_return_list_of_emails_from_string(self):
        emails = AirSpiderHelpers.extractEmailsFromText("<html><body><p>From:noreply@airspider.net</p><p>Subject:Hello user@airspider.net, here's some text</p></body></html>")
        self.assertIsInstance(emails, list)
        self.assertIn('noreply@airspider.net', emails)
        self.assertIn('user@airspider.net', emails)

    def test_extractEmailsFromText_invalid_data_should_return_list(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                emails = AirSpiderHelpers.extractEmailsFromText(invalid_type)
                self.assertIsInstance(emails, list)
    ...
    def test_extractPgpKeysFromText_should_return_list_of_pgp_keys_from_string(self):
        ...
        keys = AirSpiderHelpers.extractPgpKeysFromText(f"<html><body><p>sample{key1}sample</p><p>sample{key2}sample</p></body></html>")
    ...
    def test_extractIbansFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                ibans = AirSpiderHelpers.extractIbansFromText(invalid_type)
        ...
        for iban in ibans:
            with self.subTest(iban=iban):
                extract_ibans = AirSpiderHelpers.extractIbansFromText(iban)
    ...
    def test_extractCreditCardsFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                cards = AirSpiderHelpers.extractCreditCardsFromText(invalid_type)
                self.assertIsInstance(cards, list)

        cards = AirSpiderHelpers.extractCreditCardsFromText("airspider4111 1111 1111 1111airspider")
        self.assertIsInstance(cards, list)
        self.assertEqual(["4111111111111111"], cards)

    def test_sslDerToPem_should_return_a_certificate_as_a_string(self):
        pem = AirSpiderHelpers.sslDerToPem(
    ...
    def test_sslDerToPem_invalid_cert_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type), self.assertRaises(TypeError):
                AirSpiderHelpers.sslDerToPem(invalid_type)

    def test_extractUrlsFromText_should_extract_urls_from_string(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                cards = AirSpiderHelpers.extractUrlsFromText(invalid_type)
                self.assertIsInstance(cards, list)

        urls = AirSpiderHelpers.extractUrlsFromText("abchttps://example.airspider.net/path\rabchttp://example.airspider.net:1337/path\rabc")
        self.assertIsInstance(urls, list)
        self.assertIn("https://example.airspider.net/path", urls)
        self.assertIn("http://example.airspider.net:1337/path", urls)

    def test_extractUrlsFromRobotsTxt_should_return_list(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                robots_txt = AirSpiderHelpers.extractUrlsFromRobotsTxt(invalid_type)
                self.assertIsInstance(robots_txt, list)

        robots_txt = AirSpiderHelpers.extractUrlsFromRobotsTxt("disallow:")
        self.assertIsInstance(robots_txt, list)
        self.assertFalse(robots_txt)

        robots_txt = AirSpiderHelpers.extractUrlsFromRobotsTxt("disallow: /disallowed/path\n")
        self.assertIsInstance(robots_txt, list)
        self.assertIn("/disallowed/path", robots_txt)

    def test_sanitise_input(self):
        safe = AirSpiderHelpers.sanitiseInput("example-string")
        ...
        safe = AirSpiderHelpers.sanitiseInput("12")
        self.assertIsInstance(safe, bool)
        self.assertFalse(safe)
