...
from sfscan import startAirSpiderScanner

from airspider import AirSpiderDb
from airspider import AirSpiderHelpers
from airspider import __version__
from airspider.logger import logListenerSetup, logWorkerSetup

mp.set_start_method("spawn", force=True)


class AirSpiderWebUi:
    """AirSpider web interface."""

    lookup = TemplateLookup(directories=['airspider/templates', '.'])
    defaultConfig = dict()
    config = dict()
    token = None
    docroot = ''

    def __init__(self: 'AirSpiderWebUi', web_config: dict, config: dict, loggingQueue: 'logging.handlers.QueueListener' = None) -> None:
        """Initialize web server.

        Args:
            web_config (dict): config settings for web interface (interface, port, root path)
            config (dict): AirSpider config
            loggingQueue: TBD
        ...
        dbh = AirSpiderDb(self.defaultConfig, init=True)
        sf = AirSpider(self.defaultConfig)
    ...
    def error_page(self: 'AirSpiderWebUi') -> None:
        """Error page."""
    ...
    def error_page_401(self: 'AirSpiderWebUi', status: str, message: str, traceback: str, version: str) -> str:
    ...
    def error_page_404(self: 'AirSpiderWebUi', status: str, message: str, traceback: str, version: str) -> str:
        ...
        """
        templ = Template(filename='airspider/templates/error.tmpl', lookup=self.lookup)
        return templ.render(message='Not Found', docroot=self.docroot, status=status, version=__version__)

    def jsonify_error(self: 'AirSpiderWebUi', status: str, message: str) -> dict:
    ...
    def error(self: 'AirSpiderWebUi', message: str) -> None:
    ...
    def cleanUserInput(self: 'AirSpiderWebUi', inputList: list) -> list:
    ...
    def searchBase(self: 'AirSpiderWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search.
    ...
    def buildExcel(self: 'AirSpiderWebUi', data: list, columnNames: list, sheetNameIndex: int = 0) -> str:
    ...
    @cherrypy.expose
    def scanexportlogs(self: 'AirSpiderWebUi', id: str, dialect: str = "excel") -> bytes:
        """Get scan log
        ...
        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename=AirSpider-{id}.log.csv"
        cherrypy.response.headers['Content-Type'] = "application/csv"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return fileobj.getvalue().encode('utf-8')

    @cherrypy.expose
    def scancorrelationsexport(self: 'AirSpiderWebUi', id: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if scan_name:
                fname = f"{scan_name}-AirSpider-correlations.xlxs"
            else:
                fname = "AirSpider-correlations.xlxs"
        ...
        if filetype.lower() == 'csv':
            ...
            if scan_name:
                fname = f"{scan_name}-AirSpider-correlations.csv"
            else:
                fname = "AirSpider-correlations.csv"
    ...
    @cherrypy.expose
    def scaneventresultexport(self: 'AirSpiderWebUi', id: str, type: str, filetype: str = "csv", dialect: str = "excel") -> str:
    ...
    @cherrypy.expose
    def scaneventresultexportmulti(self: 'AirSpiderWebUi', ids: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-AirSpider.xlsx"
    ...
    @cherrypy.expose
    def scansearchresultexport(self: 'AirSpiderWebUi', id: str, eventType: str = None, value: str = None, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=AirSpider.xlsx"
            cherrypy.response.headers['Content-Type'] = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return self.buildExcel(rows, ["Updated", "Type", "Module", "Source",
                                   "F/P", "Data"], sheetNameIndex=1)
        ...
        if filetype.lower() == 'csv':
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=AirSpider.csv"
            cherrypy.response.headers['Content-Type'] = "application/csv"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return fileobj.getvalue().encode('utf-8')
    ...
    @cherrypy.expose
    def scanexportjsonmulti(self: 'AirSpiderWebUi', ids: str) -> str:
        ...
        if len(ids.split(',')) > 1 or scan_name == "":
            fname = "AirSpider.json"
        else:
            fname = scan_name + "-AirSpider.json"
    ...
    @cherrypy.expose
    def scanviz(self: 'AirSpiderWebUi', id: str, gexf: str = "0") -> str:
        ...
        if gexf == "0":
            return AirSpiderHelpers.buildGraphJson([root], data)

        if not scan_name:
            fname = "AirSpider.gexf"
        else:
            fname = scan_name + "AirSpider.gexf"

        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename={fname}"
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return AirSpiderHelpers.buildGraphGexf([root], "AirSpider Export", data)

    @cherrypy.expose
    def scanvizmulti(self: 'AirSpiderWebUi', ids: str, gexf: str = "1") -> str:
        ...
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return AirSpiderHelpers.buildGraphGexf(roots, "AirSpider Export", data)

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanopts(self: 'AirSpiderWebUi', id: str) -> dict:
    ...
    @cherrypy.expose
    def rerunscan(self: 'AirSpiderWebUi', id: str) -> None:
        """Rerun a scan.
        ...
        targetType = AirSpiderHelpers.targetTypeFromString(scantarget)
        if not targetType:
            ...
            # target type again.
            targetType = AirSpiderHelpers.targetTypeFromString(f'"{scantarget}"')

        if targetType not in ["HUMAN_NAME", "BITCOIN_ADDRESS"]:
            scantarget = scantarget.lower()

        # Start running a new scan
        scanId = AirSpiderHelpers.genScanInstanceId()
        try:
            p = mp.Process(target=startAirSpiderScanner, args=(self.loggingQueue, scanname, scanId, scantarget, targetType, modlist, cfg))
            p.daemon = True
            p.start()
    ...
    @cherrypy.expose
    def rerunscanmulti(self: 'AirSpiderWebUi', ids: str) -> str:
        """Rerun scans.
        ...
        for id in ids.split(","):
            ...
            if targetType is None:
                ...
                return self.error("Invalid target type. Could not recognize it as a target AirSpider supports.")
        ...
        templ = Template(filename='airspider/templates/scanlist.tmpl', lookup=self.lookup)
        return templ.render(rerunscans=True, docroot=self.docroot, pageid="SCANLIST", version=__version__)

    @cherrypy.expose
    def newscan(self: 'AirSpiderWebUi') -> str:
        ...
        templ = Template(filename='airspider/templates/newscan.tmpl', lookup=self.lookup)
        return templ.render(pageid='NEWSCAN', types=types, docroot=self.docroot,
                            modules=self.config['__modules__'], scanname="",
                            selectedmods="", scantarget="", version=__version__)

    @cherrypy.expose
    def clonescan(self: 'AirSpiderWebUi', id: str) -> str:
    ...
    @cherrypy.expose
    def index(self: 'AirSpiderWebUi') -> str:
        """Show landing page.

        Returns:
            str: Landing page HTML
        """
        templ = Template(filename='airspider/templates/landing.tmpl', lookup=self.lookup)
        return templ.render(pageid='LANDING', docroot=self.docroot, version=__version__)

    @cherrypy.expose
    def about(self: 'AirSpiderWebUi') -> str:
        """Show about us page.
        """
        templ = Template(filename='airspider/templates/about.tmpl', lookup=self.lookup)
        return templ.render(pageid='ABOUT', docroot=self.docroot, version=__version__)

    @cherrypy.expose
    def vision(self: 'AirSpiderWebUi') -> str:
        """Show vision and mission page.
        """
        templ = Template(filename='airspider/templates/vision.tmpl', lookup=self.lookup)
        return templ.render(pageid='VISION', docroot=self.docroot, version=__version__)

    @cherrypy.expose
    def contact(self: 'AirSpiderWebUi') -> str:
        """Show contact page.
        """
        templ = Template(filename='airspider/templates/contact.tmpl', lookup=self.lookup)
        return templ.render(pageid='CONTACT', docroot=self.docroot, version=__version__)

    @cherrypy.expose
    def scans(self: 'AirSpiderWebUi') -> str:
        """Show scan list page.
    ...
    @cherrypy.expose
    def scaninfo(self: 'AirSpiderWebUi', id: str) -> str:
        ...
        templ = Template(filename='airspider/templates/scaninfo.tmpl', lookup=self.lookup, input_encoding='utf-8')
        return templ.render(id=id, name=html.escape(res[0]), status=res[5], docroot=self.docroot, version=__version__,
                            pageid="SCANLIST")

    @cherrypy.expose
    def opts(self: 'AirSpiderWebUi', updated: str = None) -> str:
        ...
        """
        templ = Template(filename='airspider/templates/opts.tmpl', lookup=self.lookup)
    ...
    @cherrypy.expose
    def optsexport(self: 'AirSpiderWebUi', pattern: str = None) -> str:
        ...
        cherrypy.response.headers['Content-Disposition'] = 'attachment; filename="AirSpider.cfg"'
        cherrypy.response.headers['Content-Type'] = "text/plain"
        return content

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def optsraw(self: 'AirSpiderWebUi') -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scandelete(self: 'AirSpiderWebUi', id: str) -> str:
        """Delete scan(s).
    ...
    @cherrypy.expose
    def savesettings(self: 'AirSpiderWebUi', allopts: str, token: str, configFile: 'cherrypy._cpreqbody.Part' = None) -> None:
        ...
        if configFile and configFile.file:
            try:
                ...
                return self.error(f"Failed to parse input file. Was it generated from AirSpider? ({e})")
    ...
    @cherrypy.expose
    def savesettingsraw(self: 'AirSpiderWebUi', allopts: str, token: str) -> str:
        """Save settings, also used to completely reset them to default.
    ...
    def reset_settings(self: 'AirSpiderWebUi') -> bool:
    ...
    @cherrypy.expose
    def resultsetfp(self: 'AirSpiderWebUi', id: str, resultids: str, fp: str) -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def eventtypes(self: 'AirSpiderWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def modules(self: 'AirSpiderWebUi') -> list:
        """List all modules.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def correlationrules(self: 'AirSpiderWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def ping(self: 'AirSpiderWebUi') -> list:
        """For the CLI to test connectivity to this server.

        Returns:
            list: AirSpider version as JSON
        """
        return ["SUCCESS", __version__]

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def query(self: 'AirSpiderWebUi', query: str) -> str:
    ...
    @cherrypy.expose
    def startscan(self: 'AirSpiderWebUi', scanname: str, scantarget: str, modulelist: str, typelist: str, usecase: str) -> str:
        """Initiate a scan.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def stopscan(self: 'AirSpiderWebUi', id: str) -> str:
        """Stop a scan.
        ...
        for scan_id in ids:
            ...
            if scan_status != "RUNNING" and scan_status != "STARTING":
                return self.jsonify_error('400', f"The running scan is currently in the state '{scan_status}', please try again later or restart AirSpider.")
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlog(self: 'AirSpiderWebUi', id: str, limit: str = None, rowId: str = None, reverse: str = None) -> list:
        """Scan log data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanerrors(self: 'AirSpiderWebUi', id: str, limit: str = None) -> list:
        """Scan error data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlist(self: 'AirSpiderWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanstatus(self: 'AirSpiderWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scansummary(self: 'AirSpiderWebUi', id: str, by: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scancorrelations(self: 'AirSpiderWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresults(self: 'AirSpiderWebUi', id: str, eventType: str = None, filterfp: bool = False, correlationId: str = None) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresultsunique(self: 'AirSpiderWebUi', id: str, eventType: str, filterfp: bool = False) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def search(self: 'AirSpiderWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search scans.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanhistory(self: 'AirSpiderWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanelementtypediscovery(self: 'AirSpiderWebUi', id: str, eventType: str) -> dict:
        ...
        del pc['ROOT']
        retdata['tree'] = AirSpiderHelpers.dataParentChildToTree(pc)
