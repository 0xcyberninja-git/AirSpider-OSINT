...
from .threadpool import AirSpiderThreadPool
...
class AirSpiderPluginLogger(logging.Logger):
    """Used only in AirSpiderPlugin to prevent modules
...
class AirSpiderPlugin():
    """AirSpiderPlugin module object
    ...
    Attributes:
        ...
        _currentEvent (AirSpiderEvent): Current event being processed
    ...
    __scanId__ = None
    # (only used in AirSpider HX) tracking of data sources
    __dataSource__ = None
    ...
    # AirSpider object, set in each module's setup() function
    sf = None
    ...
    def log(self):
        if self._log is None:
            logging.setLoggerClass(AirSpiderPluginLogger)  # temporarily set logger class
            self._log = logging.getLogger(f"airspider.{self.__name__}")  # init AirSpiderPluginLogger
    ...
    def setTarget(self, target) -> None:
        ...
        Args:
            target (AirSpiderTarget): target

        Raises:
            TypeError: target argument was invalid type
        """
        from airspider import AirSpiderTarget

        if not isinstance(target, AirSpiderTarget):
            raise TypeError(f"target is {type(target)}; expected AirSpiderTarget")
    ...
    def tempStorage(self) -> dict:
        ...
        Note:
            Required for AirSpider HX compatibility of modules.
    ...
    def notifyListeners(self, sfEvent) -> None:
        ...
        Args:
            sfEvent (AirSpiderEvent): event

        Raises:
            TypeError: sfEvent argument was invalid type
        """

        from airspider import AirSpiderEvent
    ...
    def threadWorker(self) -> None:
        try:
            ...
            self.setDbh(AirSpiderDb(self.opts))
