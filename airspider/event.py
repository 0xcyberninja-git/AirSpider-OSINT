...
class AirSpiderEvent():
    """AirSpiderEvent object representing identified data and associated meta data.
    ...
    Attributes:
        ...
        sourceEvent (AirSpiderEvent): AirSpiderEvent that triggered this event
        sourceEventHash (str): Hash of the AirSpiderEvent event that triggered this event
    ...
    def __init__(self, eventType: str, data: str, module: str, sourceEvent: 'AirSpiderEvent') -> None:
        """Initialize AirSpider event object.

        Args:
            eventType (str): Event type, e.g. URL_FORM, RAW_DATA, etc.
            data (str): Event data, e.g. a URL, port number, webpage content, etc.
            module (str): Module from which the event originated
            sourceEvent (AirSpiderEvent): AirSpiderEvent event that triggered this event
        """
    ...
    @property
    def sourceEvent(self) -> 'AirSpiderEvent':
    ...
    @sourceEvent.setter
    def sourceEvent(self, sourceEvent: 'AirSpiderEvent') -> None:
        """Source event which lead to this event.

        Args:
            sourceEvent (AirSpiderEvent): source event
        ...
        if not isinstance(sourceEvent, AirSpiderEvent):
            raise TypeError(f"sourceEvent is {type(sourceEvent)}; expected AirSpiderEvent()")
