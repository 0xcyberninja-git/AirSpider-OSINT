...
class AirSpiderDb:
    ...
    # Queries for creating the AirSpider database
    createSchemaQueries = [
    ...
    eventDetails = [
        ['ROOT', 'Internal AirSpider Root event', 1, 'INTERNAL'],
        ...
        with self.dbhLock:
            try:
                ...
                try:
                    ...
                    raise IOError("Tried to set up the AirSpider database schema, but failed") from e
            ...
            try:
                ...
                try:
                    ...
                    raise IOError("Looks like you are running a pre-4.0 database. Unfortunately "
                                  "AirSpider wasn't able to migrate you, so you'll need to delete "
                                  "your AirSpider database in order to proceed.") from None
    ...
    def scanEventStore(self, instanceId: str, sfEvent, truncateSize: int = 0) -> None:
        ...
        Args:
            instanceId (str): scan instance ID
            sfEvent (AirSpiderEvent): event to be stored in the database
        ...
        if not isinstance(sfEvent, AirSpiderEvent):
            raise TypeError(f"sfEvent is {type(sfEvent)}; expected AirSpiderEvent()") from None
        ...
        if not isinstance(sfEvent.sourceEvent, AirSpiderEvent) and sfEvent.eventType != "ROOT":
