...
class AirSpiderCorrelator:
    """AirSpider correlation capabilities.

    Todo:
        Make the rule checking per analysis method
    """

    log = logging.getLogger("airspider.correlator")
    dbh = None
    scanId = None
    types = None
    rules = list()
    ...
    def __init__(self, dbh: AirSpiderDb, ruleset: dict, scanId: str = None) -> None:
        """Initialize AirSpider correlator engine with scan ID and ruleset.
        ...
        if not isinstance(dbh, AirSpiderDb):
            raise TypeError(f"dbh is {type(dbh)}; expected AirSpiderDb()")
    ...
    def build_db_criteria(self, matchrule: dict) -> dict:
        ...
        Returns:
            dict: criteria to be used with AirSpiderDb.scanResultEvent()
