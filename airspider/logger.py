...
from airspider import AirSpiderDb, AirSpiderHelpers


class AirSpiderSqliteLogHandler(logging.Handler):
    ...
    def makeDbh(self) -> None:
        """TBD."""
        self.dbh = AirSpiderDb(self.opts)


def logListenerSetup(loggingQueue, opts: dict = None) -> 'logging.handlers.QueueListener':
    """Create and start a AirSpider log listener in its own thread.
    ...
    log_dir = AirSpiderHelpers.logPath()
    debug_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/airspider.debug.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    if doLogging and opts is not None:
        sqlite_handler = AirSpiderSqliteLogHandler(opts)
...
def logWorkerSetup(loggingQueue) -> 'logging.Logger':
    ...
    """
    log = logging.getLogger("airspider")
