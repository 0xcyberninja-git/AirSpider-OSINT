...
class sfp_ipqualityscore(AirSpiderPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "PHONE_NUMBER":
            ...
            if data.get('leaked'):
                evt = AirSpiderEvent("EMAILADDR_COMPROMISED", f"{eventData} [Unknown]", self.__name__, event)
                self.notifyListeners(evt)
            ...
            if malicious:
                ...
                if eventName == "IP_ADDRESS":
                    ...
                    evt = AirSpiderEvent("MALICIOUS_INTERNET_NAME", maliciousDesc, self.__name__, event)
                self.notifyListeners(evt)
