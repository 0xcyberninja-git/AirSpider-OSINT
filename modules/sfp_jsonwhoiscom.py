...
class sfp_jsonwhoiscom(AirSpiderPlugin):
    ...
    def handleEvent(self, event):
        ...
        evt = AirSpiderEvent('RAW_RIR_DATA', str(res), self.__name__, event)
        ...
        for contact in contacts:
            ...
            if email:
                if AirSpiderHelpers.validEmail(email):
                    emails.append(email)
            ...
            country = AirSpiderHelpers.countryNameFromCountryCode(contact.get('country_code'))
        ...
        for email in set(emails):
            ...
            if self.getTarget().matches(mail_domain, includeChildren=True):
                ...
                evt = AirSpiderEvent("AFFILIATE_EMAILADDR", email, self.__name__, event)
                self.notifyListeners(evt)

        if eventName in ["DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = AirSpiderEvent("DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)
            ...
            if registrar:
                ...
                if registrar_name:
                    evt = AirSpiderEvent("DOMAIN_REGISTRAR", registrar_name, self.__name__, event)
                    self.notifyListeners(evt)

            for dns_provider in set(dns_providers):
                evt = AirSpiderEvent("PROVIDER_DNS", dns_provider, self.__name__, event)
                self.notifyListeners(evt)

            for name in set(names):
                evt = AirSpiderEvent("RAW_RIR_DATA", f"Possible full name {name}", self.__name__, event)
                self.notifyListeners(evt)

            for phone in set(phones):
                evt = AirSpiderEvent("PHONE_NUMBER", phone, self.__name__, event)
                self.notifyListeners(evt)
        ...
        if eventName in ["AFFILIATE_DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = AirSpiderEvent("AFFILIATE_DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)

            available = res.get('available?')
            if available:
                evt = AirSpiderEvent("AFFILIATE_DOMAIN_UNREGISTERED", eventData, self.__name__, event)
                self.notifyListeners(evt)
